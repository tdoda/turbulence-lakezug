...
function patch_setupstr_datapaper(datafname, varargin)
...
% SEB: correct the time (12 h earlier than local time...)
fname=datafname(end-12:end-10);
YY_=double(header(4)); MM_=double(header(5)); DD_=double(header(6)); hh_=double(header(7)); mm_=double(header(8)); ss_=double(header(9));
date_=datenum([YY_, MM_, DD_, hh_, mm_, ss_]);
if str2num(fname)<=87  % Adjust for the clock error
    date_=date_+0.5;
end
YY_=year(date_); MM_=month(date_); DD_=day(date_); hh_=hour(date_); mm_=minute(date_); ss_=second(date_);
if str2num(fname)<=87 % convert from CET/CEST to UTC
    date_=date_-1/24;
else
    date_=date_-2/24;
end
