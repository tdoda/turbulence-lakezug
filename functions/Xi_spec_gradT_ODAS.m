function [Xiv,XiC,Xif,KBT,fit_flag, sXif,sKBT,MAD, MADf,MADc,MLKH,LKHratio,maxK,rangeK,Kp,Kf1]=Xi_spec_gradT(pres,x0,K1,fn,KB,W,noisep, sL, sOV,Tdis,q,method,time_res,time_corr,npoles,peak_rem,plt,time,D,visco,T_dT,T_string,setupstr,int_range,folder_out)
%INPUT:
...
%dof = 0.92*8/3*NsegM;
%according to ODAS
NsegM = 2 * floor(length(x(:,1))/sL)-1; % floor(length(x)/(sL/2));
...
%             Ks = ksfact*Pr^(-0.5)*Ktest(i);
            Ks = K1;    % Note: coherently with Luketina, not considered during the 1st round
            Kf1 = max([Ks,K1]);
...
%             loglog(K,Sn+BAT,'color','k', 'linewidth',2);
        ...
        for i = 1:length(Ktest)
%             Ks = ksfact*Pr^(-0.5)*Ktest(i);
            Ks = ksfact*Pr^(-0.5)*KB00;    % Note: coherently with Luketina, Ks is based on the previous best
...
%             Ks = ksfact*Pr^(-0.5)*Ktest(i);
        ...
        if LKHratio>2 && MADf<MADc*2 ... % && KBT<maxK ... && abs(sKBT)<0.5*abs(KBT) 
                && rangeK> 0.8 ...
                && K(iKnM0)>2*Kp && Kf1<Kp
...
%             loglog(K,Sn+BAT,'color','k', 'linewidth',2);
            ...
            saveas(gcf,[folder_out '/Spec_z_comparison= ', num2str(mpres), ' m, sensor= ', T_string, '.png'])
            hold off
        end
...
% For data paper
    ...
    'PaperUnits', 'Centimeters', 'PaperSize', [16,7]);
        ...
        if cont
            ...
            s_fit=loglog(K0,Tspec(Tdis, Xif, KBT,K0,D,q), 'color',[75 97 209]/255, 'linewidth',2);
...
%         line([KBT KBT], ylim,'color','k')
        ...
        text(1.2,1e-4,['\chi_{fit} = ', num2str(Xif,'%1.3e'), '^{\circ}C^2 s^{-1}'],'horizontalalignment','left','fontsize',8)
        text(1.2,10^-4.9,['k_B = ', num2str(KBT,'%3.0f'),' cpm'],'horizontalalignment','left','fontsize',8)
        text(1.2,10^-5.7,['\epsilon_T = ', num2str(visco*D^2*(2*pi()*KBT)^4,'%1.2e'),' m^2 s^{-3}'],'horizontalalignment','left','fontsize',8)
        text(1.2,10^-6.6,['LR = ', num2str(LKHratio,'%1.1f'),'(2)'],'horizontalalignment','left','fontsize',8)
        text(1.2,10^-7.45,['MAD = ', num2str(MADf,'%1.2f'),' (', num2str(2*MADc, '%1.2f'),')'],'horizontalalignment','left','fontsize',8)

        plot(KBT,1.4E-9,'^','markeredgecolor','k','markerfacecolor','k','markersize',5)
...
%         plot(Ks,1.4E-9,'^','markeredgecolor',[75 97 209]/255,'markerfacecolor',[0.5 0.5 0.5],'markersize',5)
%         if(KL>0)
%             plot(KL,1.4E-9,'^','markeredgecolor',[75 97 209]/255,'markerfacecolor',[1 1 1],'markersize',5)
%         end
        plot(Kp,1.4E-9,'^','markeredgecolor',[75 97 209]/255,'markerfacecolor',[75 97 209]/255,'markersize',5)
        
%         plot(K(iK1),1.4E-9,'^','markeredgecolor',[75 97 209]/255,'markerfacecolor',[75 97 209]/255,'markersize',5)
%        find(PSD<Snfact*Sn | H<H_lim | K==max(K),1,'first'); 
%         text(KBT,2.2e-9,'k_B','horizontalalignment','left','rotation',90,'fontsize',8)
%         text(fn/W,2.2e-9,'0.9 k_{AA}','horizontalalignment','left','rotation',90,'fontsize',8)
%         text(K0(find(PSD0<Snfact*Sn0 ,1,'first')),2.2e-9,'SNR<1.55','horizontalalignment','left','rotation',90,'fontsize',8)
%         text(K0(find(H<H_lim  ,1,'first')),2.2e-9,'H_{lim}','horizontalalignment','left','rotation',90,'fontsize',8)
%         text(Ks,2.2e-9,'k_L(S2009)','horizontalalignment','left','rotation',90,'fontsize',8)
%         text(KL,2.2e-9,'k_L(LI2000)','horizontalalignment','left','rotation',90,'fontsize',8)
