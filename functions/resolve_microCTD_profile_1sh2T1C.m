function [BIN,SLOW,FAST] = resolve_microCTD_profile_1sh2T1C(DATA, inp, info,dataf_name,PLOT,folder_out)
    ...
    C1_dC1 = DATA.C1_dC1(iipf);
    gradC1f = DATA.gradC1(iipf);
    C2f = DATA.C2_fast(iipf);
    C2_dC2 = DATA.C2_dC2(iipf);
    gradC2f = DATA.gradC2(iipf);
    sh = DATA.sh1(iipf);
    Ax = DATA.Ax(iipf);
    Ay = DATA.Ay(iipf);
    Wf = DATA.W_fast(iipf);
...
%     depth=10000*Ps./(mrho*9.81);
    ...
    if ismember(info.prof_dir,'up') % For upward profiles, start the profile after the acceleration
        [~,idx]=min(Ws);
        n_pres = find(pres<=max(info.pmax*0.25,Ps(idx)),1,'last'); % Anyway at least 1/4 of pmax
    else
        n_pres=length(pres);
    end
    ...
    xlabel('time [min]')
    subplot(4,4,[5,9,13])
    plot(C_JAC,Ps); hold on; 
    plot(C1f,Pf); hold on;
    plot(C2f,Pf); hold on;
    legend('C','C1f','C2f','location','SouthEast')
%     plot(get(gca,'xlim'),[pmaxplot pmaxplot],'--k')
    set(gca,'ydir','reverse');
    ylabel('p(db)'); xlabel('Cond (mS cm^{-1})'); ylim(pplot);
%     subplot(4,4,[6,10,14])
%     plot(AA(:,1),Pf); hold on
%     plot(AA(:,2),Pf); set(gca,'ydir','reverse');
%     xlabel('Acc (1/s)')
    subplot(4,4,[7,11,15]);
    plot(T_JAC,Ps,'-k'); hold on
%     plot(get(gca,'xlim'),[pmaxplot pmaxplot],'--k')
    set(gca,'ydir','reverse');
    xlabel('T (°C)'); ylim(pplot)
    set(gca,'yticklabel',[])
...
%         'xColor','r','yColor','k');   
%     set(ax1,'box','off')    
%     line(Incl_X,Ps,'color','r'); set(gca,'ydir','reverse');
%     xlabel('Inclination [°]')
...
%     %     legend('sh','sh_{desp}','sh_{HP}')
    subplot(4,4,[6,10,14])
    plot(gradT1f,Pf); hold on
    plot(gradT2f+5,Pf); hold on
    plot(gradC1f+10,Pf); hold on
    plot(gradC2f+15,Pf); hold on
    legend('gradT1','gradT2','gradC1','gradC2','location','SouthEast')
%     plot(get(gca,'xlim'),[pmaxplot pmaxplot],'--k')
    set(gca,'ydir','reverse');
%     plot([-200 -200],[min(Pf) max(Pf)],'--k');    plot([200 200],[min(Pf) max(Pf)],'--k');
%     xlim([-500 500]);  
    xlabel('grad T, grad C');  xlim([-5 20]); ylim(pplot);
    ...
    BIN.alpha = nan(1,length(pres));
    BIN.dCdT = nan(1,length(pres));
    ...
    for i = 1:n_pres; %length(pres)
        ...
        % calculate dC/dT [uS/cm/deg C], to be used when salinity gradients are
        % low/negligible to correct the gradient spectrum of conductivity
        [~,BIN.alpha(i),BIN.dCdT(i)]=conductivity_20(T_JAC(jps),Cmatch(jps)*1000);
...
%             [BIN.epsSH(i), BIN.W(i), BIN.fit_flag_sh(i), BIN.MADsh(i), BIN.MADcsh(i)] = dis_spec_ODAS(sh_hp(jp), AAxy(jp,:), fsf, fss, WW, T_JAC_fast(jp),timef(jp), Pf(jp), visco,Nfft,  PLOT, folder_out);
        ...
        elseif strcmp(info.Nasmyth_spec,'EPFL')
            [BIN.epsSH(i), BIN.W(i), BIN.fit_flag_sh(i), BIN.MADsh(i), BIN.MADcsh(i)] = ...
                dis_spec_EPFL(Pf(jp),sh_hp(jp),AA(jp),0.1,14,30,visco,WW, Nfft, overlap, PLOT,folder_out, info.noise_corr);
        end
...
%             [BIN.Xiv2(i),BIN.Xic2(i),BIN.Xif2(i),BIN.KB2(i),BIN.fit_flag_T2(i), BIN.sXif2(i), BIN.sKB2(i), BIN.MAD2(i), BIN.MADf2(i),BIN.MADc2(i),BIN.LKH1(i), BIN.LKHratio2(i), BIN.maxK2(i)] =Xi_spec_gradT(Pf(jp),gradT2f(jp),info.minKT,info.fAA,BIN.KBSH(i),WW,info.noisep_T2,  Nfft, overlap,info.Tspec,info.Tmethod,info.time_res, info.peak_rem_T2,PLOT);
            [BIN.Xiv2(i),BIN.Xic2(i),BIN.Xif2(i),BIN.KB2(i),BIN.fit_flag_T2(i), BIN.sXif2(i), BIN.sKB2(i), BIN.MAD2(i), BIN.MADf2(i),BIN.MADc2(i),BIN.LKH1(i), BIN.LKHratio2(i), BIN.maxK2(i), BIN.rangeK2(i)] =...
...
%             [BIN.Xiv2(i),BIN.Xic2(i),BIN.Xif2(i),BIN.KB2(i),BIN.fit_flag_T2(i), BIN.sXif2(i), BIN.sKB2(i), BIN.MAD2(i), BIN.MADf2(i),BIN.MADc2(i),BIN.LKH1(i), BIN.LKHratio2(i), BIN.maxK2(i)] =Xi_spec(Pf(jp),T2f_hp(jp),info.minKT,info.fAA,BIN.KBSH(i),WW,info.noisep_T2,  Nfft, overlap,info.Tspec,info.Tmethod,info.time_res, info.peak_rem_T2,PLOT,timef(jp));
%             [BIN.Xiv2(i),BIN.Xic2(i),BIN.Xif2(i),BIN.KB2(i),BIN.fit_flag_T2(i), BIN.sXif2(i), BIN.sKB2(i), BIN.MAD2(i), BIN.MADf2(i),BIN.MADc2(i),BIN.LKH1(i), BIN.LKHratio2(i), BIN.maxK2(i)] =Xi_spec_gradT(Pf(jp),gradT2f(jp),info.minKT,info.fAA,BIN.KBSH(i),WW,info.noisep_T2,  Nfft, overlap,info.Tspec,info.Tmethod,info.time_res, info.peak_rem_T2,PLOT);
            [BIN.XivC1(i),BIN.XicC1(i),BIN.XifC1(i),BIN.KBC1(i),BIN.fit_flag_C1(i), BIN.sXifC1(i), BIN.sKBC1(i), BIN.MADC1(i), BIN.MADf2C1(i),BIN.MADcC1(i),BIN.LKHC1(i), BIN.LKHratioC1(i), BIN.maxKC1(i), BIN.rangeKC1(i)] =...
                Xi_spec_gradC_ODAS(Pf(jp),gradC1f(jp),info.minKT,info.fAA,BIN.KBSH(i),WW,info.noisep_T2, Nfft, overlap,info.Tspec,info.q,info.Tmethod,0,info.time_corr,info.npoles,info.peak_rem_T2,PLOT,timef(jp),D,visco,T2_dT2,'T2_dT2',DATA.setupfilestr,info.int_range,folder_out);
         %%%%%%USE gradC1f(jp)*1000/BIN.dCdT(i)
        ...
        figure
        subplot(1,4,1); plot(Cmatch,Ps); set(gca,'Ydir','reverse')
        subplot(1,4,2); plot(T_JAC,Ps); set(gca,'Ydir','reverse')
        subplot(1,4,3); plot(Ss,Ps); set(gca,'Ydir','reverse')
        subplot(1,4,4); plot(BIN.dCdT,pres); set(gca,'Ydir','reverse')
