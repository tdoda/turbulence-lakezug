...
%with: 
%C25_to_S = 0.79
%fT_function25(t)=1.8692 -0.055181*t + 0.0011096*t.^2-1.1762e-5*t.^3



function [BIN,SLOW,FAST,AA,accel,accel_opt,acc_bin] = resolve_microCTD_profile(DATA, inp, info,dataf_name,PLOT,folder_out)
...
%     BIN.LTc = sqrt(pres_av(Ps,Lc.^2,pres,info.dpD,0));
    ...
    BIN.acc = -mean_grad_time(Pf,Wf,pres,info.dpD);  % instrument acceleration
    acc_bin = BIN.acc;
    
    accel = nan((length(Wf)-1),1);
    indaccel = [floor(length(Wf)*0.05):ceil(length(Wf)*0.95)];

    for i=1:(length(Wf)-1)
        accel(i)=512*(Wf(i+1)-Wf(i));
    end
    accel_opt = accel(indaccel);
    %plots raw 
    figure(1)
    clf
    subplot(4,4,[1 4])
...
%     plot(sh2_hp,Pf)
	...
	BIN.K1sh1 = nan(1,length(pres));
    BIN.K3sh1 = nan(1,length(pres));
	...
	BIN.K1sh2 = nan(1,length(pres));
    BIN.K3sh2 = nan(1,length(pres));
...
%         try
%             if strcmp(info.Nasmyth_spec,'ODAS') % to be verified
% %                 [BIN.epsSH1(i), BIN.W(i), BIN.fit_flag_sh1(i), BIN.MADsh1(i), BIN.MADcsh1(i)] = ...
% %                     dis_spec_ODAS([sh1_hp(jp) sh2_hp(jp)], AA(jp,:), fsf, fss, WW, T_JAC_fast(jp),timef(jp), Pf(jp), visco,Nfft, PLOT, folder_out, info.noise_corr);
%             elseif strcmp(info.Nasmyth_spec,'EPFL')
%                 [BIN.epsSH1(i), BIN.W(i), BIN.fit_flag_sh1(i), BIN.MADsh1(i), BIN.MADcsh1(i), BIN.K1sh1(i),BIN.K3sh1(i)] = ...
%                     dis_spec_EPFL_paper(Pf(jp),[sh1_hp(jp) sh2_hp(jp)],AA(jp,:),0.1,14,info.fAA,visco,WW, Nfft, overlap, PLOT,folder_out, info.noise_corr,'sh1',Pf(jp),T1f(jp));
%             end
...
%         try
%             if strcmp(info.Nasmyth_spec,'ODAS') % to be verified
% %                 [BIN.epsSH2(i), ~, BIN.fit_flag_sh2(i), BIN.MADsh2(i), BIN.MADcsh2(i)] = ...
% %                     dis_spec_ODAS([sh1_hp(jp) sh2_hp(jp)], AA(jp,2), fsf, fss, WW, T_JAC_fast(jp),timef(jp), Pf(jp), visco,Nfft, PLOT, folder_out, info.noise_corr);
%             elseif strcmp(info.Nasmyth_spec,'EPFL')
%                 [BIN.epsSH2(i), ~, BIN.fit_flag_sh2(i), BIN.MADsh2(i), BIN.MADcsh2(i), BIN.K1sh2(i),BIN.K3sh2(i)] = ...
%                     dis_spec_EPFL_paper(Pf(jp),[sh1_hp(jp) sh2_hp(jp)],AA(jp,:),0.1,14,info.fAA,visco,WW, Nfft, overlap, PLOT,folder_out, info.noise_corr,'sh2',Pf(jp),T1f(jp));
%             end
%             BIN.KBSH2(i)=1/(2*pi())*(BIN.epsSH2(i)/(visco*D^2))^(1/4);
%         end
%    
% % %                  [BIN.epsSH21(i), BIN.MADsh21(i), BIN.MADcsh21(i), BIN.fit_flag_sh21(i), BIN.K1sh2(i),BIN.K3sh21(i)] = ...
% % %                     TKE_dis_spec(Pf(jp),[sh1_hp(jp) sh2_hp(jp)],AA(jp,:),0.1,14,info.fAA,visco,WW, Nfft, overlap, info.noise_corr,'sh2',1,Pf(jp),T1f(jp));
%    
%                 
%         if (BIN.fit_flag_sh1(i)==1 && BIN.fit_flag_sh2(i)==0)
%             meanKBSH=BIN.KBSH1(i);
%         elseif (BIN.fit_flag_sh1(i)==0 && BIN.fit_flag_sh2(i)==1)
%             meanKBSH=BIN.KBSH2(i);
%         else % if both accepted or both rejected (anyway I want to have a value)
%             meanKBSH=mean([BIN.KBSH1(i), BIN.KBSH2(i)]);
%         end
%         
%         %% FP07 calculations
%         try
...
%                 Xi_spec_gradT_ODAS_paper(Pf(jp),gradT1f(jp),info.minKT,info.fAA,meanKBSH,WW,info.noisep_T1, Nfft, overlap,info.Tspec,info.q,info.Tmethod,info.time_res,info.time_corr,info.npoles,info.peak_rem_T1,PLOT,timef(jp),D,visco,T1_dT1,'T1_dT1',DATA.setupfilestr,info.int_range,folder_out,Pf(jp),T1f(jp));
% 
% % %                         [BIN.Xiv11(i),BIN.Xic11(i),BIN.Xif11(i),BIN.KB11(i),eps_TT, BIN.MAD11(i), BIN.MADf11(i),BIN.MADc11(i),BIN.LKHratio11(i), BIN.minK11(i), BIN.maxK11(i), BIN.rangeK11(i), BIN.Kp11(i),BIN.fit_flag_T11(i)] =...
% % %                 gradT_dis_spec(Pf(jp),gradT1f(jp),info.minKT,info.fAA,meanKBSH,WW, Nfft, overlap,info.Tspec,info.q,info.time_res,info.time_corr,info.npoles,info.int_range,D,visco,T1_dT1,'T1_dT1',DATA.setupfilestr,1,Pf(jp),T1f(jp));
% 
...
%                 Xi_spec_gradT_ODAS_paper(Pf(jp),gradT2f(jp),info.minKT,info.fAA,meanKBSH,WW,info.noisep_T2, Nfft, overlap,info.Tspec,info.q,info.Tmethod,info.time_res,info.time_corr,info.npoles,info.peak_rem_T2,PLOT,timef(jp),D,visco,T2_dT2,'T2_dT2',DATA.setupfilestr,info.int_range,folder_out,Pf(jp),T1f(jp));
%          
