...
%% The only difference relative to the original get_diss_odas (v 4.4) is how the viscosity is calculated:
%  it is not the viscosity for sea water as in the original version, but it is calculated based on actual value of S
%  The value of the viscosity is passed through infoD
%  All changes are marked with % EPFL change
...
function diss = get_diss_odas_EPFL(shear, A, varargin)
...
addParamValue(p, 'viscosity',   val_data);   % EPFL change  SEB
addParamValue(p, 'noise_corr',  val_data);   % EPFL change  SEB
...
UVW          = p.Results.UVW;
% goodman      = p.Results.goodman;   % EPFL change: not needed because we now use noise_corr SEB
...
num_of_ffts = 2 * floor(length(select) / fft_length) - 1;   % EPFL: -1 added SEB
...
while select(end) <= size(shear,1)
    ...
    % EPFL change SEB: implemented the possibility to choose the noise correction algorithm
%     if goodman
    if strcmpi(p.Results.noise_corr,'goodman')
        [P_sh_clean, AA, P_sh, UA, F] = ...
        clean_shear_spec(A(select,:), shear(select,:), fft_length, fs_fast);
    elseif  strcmpi(p.Results.noise_corr,'none')       
    ...
    if size(P_sh,2) > 1 && size(AA,2) > 1
        UA         = permute(UA,         [3 1 2]);
    % EPFL change: since we have 1 sh and 2 piezo acc SEB
    elseif size(P_sh,2) ==1 && size(AA,2) > 1 && strcmpi(p.Results.noise_corr,'none')
        UA         = UA(:,1,:);
        UA         = permute(UA,         [3 1 2]);
    % EPFL change    
    else
...
%    nu     = visc35(mean_T);  % EPFL change SEB
     nu = p.Results.viscosity; % EPFL change SEB
    ...
    for column_index = 1:num_of_shear
        ...
        if e_1 < e_inertial_sr % Use the variance method
            ...
            %%%% EPFL %%%%%
            pr1 = log10(K_95); % SEB just discard the polyfit approach!
            %%%% EPFL %%%%%
