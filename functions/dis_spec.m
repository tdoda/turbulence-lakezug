function [eps,Kc,epsN,KcN,MAD,MADf, MADc]=dis_spec(pres,x0,px0,K1,K2,Kn,visco, sL, sOV, plt)
    ...
    px0 = px0(I);
    pres = pres(I);
    x1 = detrend(x0,'linear');
    px = detrend(px0,'linear');
    
    %deletes peaks
    t = 1:length(x1);
    sx = std(x1);
    iibad = find(abs(x1)>3*sx);
    x2 = x1;
    x2(iibad) = nan;
    iif = find(isfinite(x2));
    x = interp1(t(iif),x2(iif), t);
    ...
    [PSD0,K] = pwelch(x-nanmean(x),hann(sL),sOV,[],Fs,'onesided');
    if sum(abs(px))>0
        [PSDps, ~] = pwelch(px-nanmean(px),hann(sL),sOV,[],Fs,'onesided');
        [CPS, ~] = cpsd(x-nanmean(x),px-nanmean(px),hann(sL),sOV,[],Fs,'onesided');
        H = CPS./PSDps;
...
%        eps=10.^log_eps;
       ...
       MAD = meanabsdev( PSD(iK1:iKn), NAS(iK1:iKn));
       MADf = meanabsdev( PSD(iK1:iKn), NAS2(iK1:iKn));
       ...
       if plt(2)~=0
            ...
            plot(x1,pres1,'color',[0.5,0.5,0.5])
            hold on
            plot(x,pres,'k')
            line(2.7*[sx,sx],ylim,'color','k','linestyle','--')
            line(-2.7*[sx,sx],ylim,'color','k','linestyle','--')
            axis ij
            grid('on')
            xlabel('shear (s^{-1})')
            ...
            loglog(logspace(0,3,100),nasmyth(logspace(-11,-1,12),visco,logspace(0,3,100)),'color',[0.60 0.60 0.60])
            hold on
            for i=1:12
