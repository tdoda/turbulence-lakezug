function [BIN,SLOW] = resolve_VMP_profile(DATA, inp, info,dataf_name,PLOT)
    ...
    dens = sigma_p(Ps, T_JAC, Ss)+1000;
    %sgt = sigmat(T_JAC, Ss)+1000;
    sgt = freshwater_density(T_JAC,Ss);
    ...
    BIN.N2 = -9.81*mean_grad(Ps,sgt,pres,info.dpD)/1000;
    ...
    for i = 1:length(pres)
        ...
        %FP07 calculations
        try
            [BIN.Xiv1(i),BIN.Xif1(i),BIN.KB1(i),BIN.fit_flag_T1(i),BIN.sXif1(i), BIN.sKB1(i), BIN.MADf1(i),BIN.MADc1(i),BIN.LKH1(i), BIN.LKHratio1(i), BIN.maxK1(i)] =Xi_spec_no_shear(Pf(jp),T1f_hp(jp),info.minKT,info.fAA,mean(abs(Wf(jp))),info.noisep_T1,  Nfft, overlap,info.Tspec,info.Tmethod,PLOT);
            BIN.epsT1(i) = visco*D^2*(2*pi()*BIN.KB1(i))^4;
        end
        
        try
            [BIN.Xiv2(i),BIN.Xif2(i),BIN.KB2(i),BIN.fit_flag_T2(i), BIN.sXif2(i), BIN.sKB2(i), BIN.MADf2(i),BIN.MADc2(i),BIN.LKH1(i), BIN.LKHratio2(i), BIN.maxK2(i)] =Xi_spec_no_shear(Pf(jp),T2f_hp(jp),info.minKT,info.fAA, mean(abs(Wf(jp))),info.noisep_T2,  Nfft, overlap,info.Tspec,info.Tmethod,PLOT);
    ...
    set(ax2,'box','off')
    plot(sgt, Ps,'r', 'parent' , ax2,'linewidth',2)
    ...
    hold on
    plot(BIN.KTf2,BIN.pres,'o-','linewidth',2,'markersize',4)
    plot(BIN.KOsb1,BIN.pres,'o-','linewidth',2,'markersize',4)
    plot(BIN.KOsb2,BIN.pres,'o-','linewidth',2,'markersize',4)
...
%     ylim([pmin,pmax])
     ...
     hold on
     plot(BIN.LO1,BIN.pres,'o-','linewidth',2,'markersize',4)
     plot(BIN.LO2,BIN.pres,'o-','linewidth',2,'markersize',4)
