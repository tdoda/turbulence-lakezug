...
data_folder_rel=os.path.join('..','..','..','data','microCTD',campaign_name,'Level2')

# Other option without "..":
script_path=os.path.dirname(os.path.abspath(__file__))
data_folder=os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(script_path))),'data','microCTD',campaign_name,'Level2')
...
#%% Thorpe scale
    ...
    dz_botrem=1
    dz_toprem=10
    DATA[kf]["THORPE"]=[None]*len(DATA[kf]["SLOW"])
    for kprof in range(len(DATA[kf]["SLOW"])):
        depthval=DATA[kf]["FAST"][kprof]["depth"].copy()
        tempval=DATA[kf]["FAST"][kprof]["fast_T1"].copy()
        ...
        DATA[kf]["THORPE"][kprof]={"depth":depthval,"temp_sorted":temp_sorted_nan,"idx_overturns":idx_overturns,
...
for kf in range(len(DATA)):
    nprof=len(DATA[kf]["SLOW"])
    dataprof_slow=DATA[kf]["SLOW"][nprof-1]
    dataprof_fast=DATA[kf]["FAST"][nprof-1]
    ax[0].plot(dataprof_slow["temperature"],dataprof_slow["depth"],linewidth=1)
    
    ax[1].plot(dataprof_fast["fast_T1"],dataprof_fast["depth"],linewidth=1)
    
    ax[2].plot(dataprof_fast["grad_T1"]+kf*2,dataprof_fast["depth"],linewidth=1)
    
    ax[3].plot(dataprof_fast["fast_S1"]+kf,dataprof_fast["depth"],linewidth=1)
...
ax[2].set_xlim(-1,2*len(DATA))
ax[3].set_xlim(-1,len(DATA))
ax[0].invert_yaxis()
...
for kf in range(len(DATA)):
    nprof=len(DATA[kf]["SLOW"])
    dataprof_binned=DATA[kf]["BINNED"][nprof-1]
    
    ax[0].plot(dataprof_binned["LT"],dataprof_binned["depth"],'-',linewidth=1)
    
    ax[1].plot(dataprof_binned["Xi_T1"],dataprof_binned["depth"],'-',linewidth=1)
    
    ax[2].plot(dataprof_binned["eps_T1"],dataprof_binned["depth"],'-',linewidth=1)
    
    ax[3].plot(dataprof_binned["eps_S1"],dataprof_binned["depth"],'-',linewidth=1)
...
ax[0].set_xticks(np.arange(0, 20, 4))
